...
class EdgeGroups {
  public:
    ...
    bool add_non_config_edge(int from_node, int to_node) {
        return graph_[from_node].edges.insert(to_node).second &&
            graph_[to_node].edges.insert(from_node).second;
    }
    ...
    void create_sets() {
        ...
        for (auto& node : graph_) {
            if (node.second.set == OPEN) {
                node.second.set = num_groups++;
                add_connected_group(node.second);
            }
        }
        ...
        rr_non_config_node_sets_map_.resize(num_groups);
        for (const auto& node : graph_) {
  ...
  private:
    ...
    // Perform a BFS traversal marking everything reachable with the same set id
    void add_connected_group(const node_data& node) {
        // q contains nodes with edges to mark with node.set
        // The set for each node must be set before pushing it into the queue
        std::queue<const node_data*> q;
        q.push(&node);
        while(!q.empty()) {
            auto top = q.front();
            q.pop();
